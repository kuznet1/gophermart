...
func main() {
	...
	if err != nil {
		Log.Fatal("unable to parse config", zap.Error(err))
	}

	repo, err := repository.NewRepo(cfg)
	if err != nil {
		Log.Fatal("unable to create database storage", zap.Error(err))
	}
	...
	Log.Info("Gophermart service is running at " + cfg.RunAddress)
	Log.Fatal(http.ListenAndServe(cfg.RunAddress, h.Router()).Error())
}
